...
from .models import Patient, PatientVisit, ScanImage, ScanReport
from .serializers import CustomUserSerializer, PatientSerializer, PatientVisitSerializer, ScanReportSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not existing_user:
            userobj = User.objects.create(
                phone_number=phone_number, first_name=first_name, last_name=last_name, email=email, hospital=hospital
            )
...
class ScanReportViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = ScanReport.objects.filter(
            Q(patient_id=patient_id) | Q(patient_visit__patient_id=patient_id)
        ).order_by("id")
    ...
    def create(self, request, *args, **kwargs):
        ...
        for scan_image_data in scan_images:
            scan_image = ScanImage(scan_report=patient_report, image_file=scan_image_data)
            scan_image.save()
        return Response({"success": True}, status=status.HTTP_201_CREATED)
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        for scan_image_data in scan_images:
            scan_image = ScanImage(scan_report=instance, image_file=scan_image_data)
            scan_image.save()
...
class PatientVisitViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if follow_up_appointments_str:
            follow_up_appointments = timezone.make_aware(
                datetime.datetime.strptime(follow_up_appointments_str, "%Y-%m-%d %H:%M")
            )
